...
def calculate_tpr_fpr_with_f1_score(dataloader, model, result_path):
    ...
    for data, target, filename in tqdm(dataloader, dynamic_ncols=True, desc="finding threshold"):
        ...
        input_img = Image.open(filename[0]).convert('L')
    
        input_img = input_img.crop((l, 0, l+cropsize, 416))
        input_tensor = trans(input_img).unsqueeze(0)
        
        unfolded = torch.nn.functional.unfold(input_tensor, kernel_size=(8, 8), stride=(8, 8)).permute(0, 2, 1).reshape(-1, 64).cuda()
        dist = torch.cdist(
                unfolded.unsqueeze(0), coreset.embedding_coreset, 2,
        ...
        resize_dist = cv2.resize(resize_dist.numpy(), (output.shape[1], output.shape[0]), interpolation=cv2.INTER_NEAREST)
        
        new_output = output.detach().numpy().copy()
        
        y_idx, x_idx = np.where(np.logical_and(resize_dist < resize_dist.mean().item(), new_output >= fastflow_threshold))
    ...
    df.to_excel('fastflow_result.xlsx')
    return
