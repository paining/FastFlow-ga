...
def calculate_tpr_fpr_with_f1_score(dataloader, model, result_path):
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(result_path, "image-distribution.png"))
    plt.close(fig)
...
def train(args):
    ...
    if args.exp_name is None:
        ...
        checkpoint_dir = os.path.join(const.CHECKPOINT_DIR, args.exp_name)
...
def parse_args():
    ...
    parser.add_argument("--exp_name", type=str, default=None, help="save folder name")
